...
读实现、结果必然漂移（同 `方案-组策略可编辑-自包含-2026-09-17.md` 对写实现
N1 的裁定：只保留一份）。生产只认 LDAP 一条；GPMC 那份的角色是**独立的
