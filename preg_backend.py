...
* **不连域、不碰 SYSVOL、不导入 PyQt、不碰文件系统** —— 本模块**只收字节、只回字节**，
  ...
  理由见下面那条。
* 🔴 **`read_preg_file` 已删除**（2026-09-18，主理人拍板）：它原是「读文件 ＋ `parse_preg`」
  ...
  **原文快照**在 `桌面/AD域管理工具-快照/pre-delete-read-preg-file-20260918-1310/`
  （整目录覆盖回去即可回滚）。⚠️ 这个名字**现在在本仓库里再也查不到**，那不是笔误。
* **编码（`build_preg`）也在这个模块** —— 理由只有一个：`parse_preg` 与 `build_preg`
  必须是**同一份格式知识的两个方向**。分成两个模块，改了一边忘另一边，
  表现就是「读得出来、写回去变了样」，而那正是要根治的病。
* 🔴 **2026-09-18：本模块的「编辑」那一半已移出仓库**（主理人裁定「**这个工具不做
  组策略编辑**」）⇒ 写入口、原地替换、删条目、按 `(键, 值名)` 查找这四项**均已删除**；
  **原文快照**在
  `桌面/AD域管理工具-快照/pre-archive-preg-admx-write-side-20260918-1230/preg_backend.py`
  （整目录覆盖回去即可回滚）。⚠️ 那四个名字**现在在本仓库里再也查不到**，那不是笔误。
