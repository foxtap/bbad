...
ADUC 对齐（批次 1~3，规格见 Obsidian《10-ADUC功能对齐规格》）：
...
class BrowserPage(QWidget):
    ...
    def _build_list_side(self) -> QWidget:
        ...
        # 「强制复制同步」—— 2026-09-17 主理人二次裁定后**补到页面里**。
        #
        # 背景（别再走回头路）：09-17 主理人问的原话是「能不能在软件**页面里**
    ...
    def notify(self, text: str, level: str = "info") -> None:
        ...
        ⚠️ **连 `ok` / `info` 也记**，不做"只记错误"的过滤：排障时主理人引用的
    ...
    def _announce_sync(self, key: tuple, text: str, level: str, *,
        ...
        **结构性**的 —— 建号得到它、删号得到它、建 OU 还是它。
        ...
        ⚠️ **日志不去重**：`.workbuddy` 的排障依赖 `app.log` 里**每一次**都有那一行。
