...
> 本仓库是**发布用子集**：只含程序本体、`docs/` 与 `screenshots/`。
> 开发期的测试与验收装置留在作者本地，不进公开仓。
...
| 文档 | 内容 |
|---|---|
| [上线就绪评估](docs/上线就绪评估-2026-09-14.md) | 上线前逐项核对 |
| [对标 ADUC 差异清单](docs/对标ADUC-差异清单-2026-09-15.md) | 相对 `dsa.msc` 做了什么、没做什么 |
| [设计：组策略只读](docs/设计-组策略只读-2026-09-15.md) | 组策略为什么只读、边界在哪 |
| [连接排障与日志采集](docs/连接排障-日志采集-2026-09-15.md) | 连不上时按什么顺序查 |
| [事故：函数体被吞](docs/事故-函数体被吞-2026-09-15.md) | 一次缩进事故与它换来的静态守卫 |
| [回归慢的根因：界面测试窗口泄漏](docs/回归慢的根因-界面测试窗口泄漏-2026-09-15.md) | 测试越跑越慢的定位过程 |
...
> 这不是 MIT。MIT 允许他人拿去商用、甚至闭源转售；本协议**明确保留商业权利**。
> 选 MIT 与本工具的定位（作者自用、公开留档）是矛盾的，所以没有采用。
